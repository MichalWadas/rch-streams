...
RUNTIME_NAME=$(echo $DB_HOST | awk -F'-' '{print$2}')
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  ...
  kubectl run mysql --attach=true --rm --image=alpine -- /bin/sh -c 'apk add mysql-client; mysql -u'$DB_USERNAME' -h '$DB_HOST' -p'$DB_PASSWORD' -Bse "DROP SCHEMA \`'$ACESS_CONTROL_DB_NAME'\`; commit; CREATE SCHEMA \`'$ACESS_CONTROL_DB_NAME'\`; commit; DROP SCHEMA \`'$ARRANGEMENT_MANAGER_DB_NAME'\`; commit; CREATE SCHEMA \`'$ARRANGEMENT_MANAGER_DB_NAME'\`; commit; DROP SCHEMA \`'$USER_MANAGER_DB_NAME'\`; commit; CREATE SCHEMA \`'$USER_MANAGER_DB_NAME'\`; commit; DROP SCHEMA \`'$TRANSACTION_MANAGER_DB_NAME'\`; commit; CREATE SCHEMA \`'$TRANSACTION_MANAGER_DB_NAME'\`; commit;" '
